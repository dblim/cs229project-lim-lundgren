...
data, opens = preprocess_arima(minutizer(combine_ts(tickers), split=5), tickers)
n, _ = data.shape
...
train_predictions = model_fit.forecast(steps=exog_x.shape[0], exog=exog_x.values)
train_residuals = np.subtract(np.array(train_predictions).reshape(endog_y.shape[0], len(tickers)), endog_y.values)
train_residuals = pd.DataFrame(train_residuals, columns=tickers)
train_residuals.to_csv('../output/ARIMA_results/VARMAX_train_residuals.csv')
...
test_residuals.to_csv('../output/ARIMA_results/VARMAX_test_residuals.csv')
